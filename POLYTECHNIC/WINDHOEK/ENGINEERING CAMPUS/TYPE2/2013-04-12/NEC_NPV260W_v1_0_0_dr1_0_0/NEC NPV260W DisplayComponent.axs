...
(*********************************************************************)
MODULE_NAME = 'NEC NPV260W DisplayComponent' (dev vdvDev[], dev dvTP, INTEGER nDevice, INTEGER nPages[])
(***********************************************************)
(* System Type : NetLinx                                   *)
...
#include 'NEC NPV260W ComponentInclude.axi'
...
// Channels
BTN_GET_ACTIVE_WINDOW           = 1359  // Button: getActiveWindow
BTN_GET_ASPECT                  = 1350  // Button: getAspectRatio
BTN_GET_VIDEO_TYPE              = 1366  // Button: getVideoType
BTN_ACTIVE_WINDOW_SUB           = 1358  // Button: setActiveWindow - Sub
BTN_ACTIVE_WINDOW_RIGHT         = 1357  // Button: setActiveWindow - Right
BTN_ACTIVE_WINDOW_MAIN          = 1355  // Button: setActiveWindow - Main
BTN_ACTIVE_WINDOW_LEFT          = 1356  // Button: setActiveWindow - Left
BTN_VIDEO_TYPE_NTSC             = 1361  // Button: setVideoType - NTSC
BTN_VIDEO_TYPE_PAL              = 1362  // Button: setVideoType - PAL
BTN_VIDEO_TYPE_SECAM            = 1363  // Button: setVideoType - SECAM
BTN_VIDEO_TYPE_AUTO             = 1360  // Button: setVideoType - Auto
...
char sACTIVEWINDOW[MAX_ZONE][20] = { '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '' }
char sVIDEOTYPE[MAX_ZONE][20] = { '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '' }
...
DEFINE_FUNCTION processOnlineEvent()
{
	...
	for (nLoop = 1; nLoop <= length_array(vdvDev); nLoop++)
	{
		...
		SEND_COMMAND vdvDev[nLoop], '?ASPECTRATIOSELECT'
		println("'send_command ',dpstoa(vdvDev[nLoop]),', ',39,'?ASPECTRATIOSELECT',39")
	}
...
DATA_EVENT[vdvDev]
{
    COMMAND :
    {
		...
		if (nZone == nCurrentZone)
		{
            ...
            // SNAPI::ACTIVEWINDOW-RIGHT
            CASE 'ACTIVEWINDOW' :
            {
                sACTIVEWINDOW[nZone] = DuetParseCmdParam(cCmd)
	    ...
	    CASE 'ASPECTRATIOPROPERTIES' :
	    {
            ...
            CASE 'VIDEOTYPE' :
            {
                sVIDEOTYPE[nZone] = DuetParseCmdParam(cCmd)
...
//
//---------------------------------------------------------------------------------
BUTTON_EVENT[dvTP, BTN_ACTIVE_WINDOW_LEFT]
{
    push:
    {
        if (bActiveComponent)
        {
            send_command vdvDev[nCurrentZone], 'ACTIVEWINDOW-LEFT'
            println ("'send_command ',dpstoa(vdvDev[nCurrentZone]),', ',39,'ACTIVEWINDOW-LEFT',39")
        }
    }
}
...
//
//---------------------------------------------------------------------------------
BUTTON_EVENT[dvTP, BTN_ACTIVE_WINDOW_MAIN]
{
    push:
    {
        if (bActiveComponent)
        {
            send_command vdvDev[nCurrentZone], 'ACTIVEWINDOW-MAIN'
            println ("'send_command ',dpstoa(vdvDev[nCurrentZone]),', ',39,'ACTIVEWINDOW-MAIN',39")
        }
    }
}
...
//
//---------------------------------------------------------------------------------
BUTTON_EVENT[dvTP, BTN_ACTIVE_WINDOW_RIGHT]
{
    push:
    {
        if (bActiveComponent)
        {
            send_command vdvDev[nCurrentZone], 'ACTIVEWINDOW-RIGHT'
            println ("'send_command ',dpstoa(vdvDev[nCurrentZone]),', ',39,'ACTIVEWINDOW-RIGHT',39")
        }
    }
}
...
//
//---------------------------------------------------------------------------------
BUTTON_EVENT[dvTP, BTN_ACTIVE_WINDOW_SUB]
{
    push:
    {
        if (bActiveComponent)
        {
            send_command vdvDev[nCurrentZone], 'ACTIVEWINDOW-SUB'
            println ("'send_command ',dpstoa(vdvDev[nCurrentZone]),', ',39,'ACTIVEWINDOW-SUB',39")
        }
    }
}
...
//
//---------------------------------------------------------------------------------
BUTTON_EVENT[dvTP, BTN_GET_VIDEO_TYPE]
{
    push:
    {
        if (bActiveComponent)
        {
            send_command vdvDev[nCurrentZone], '?VIDEOTYPE'
            println ("'send_command ',dpstoa(vdvDev[nCurrentZone]),', ',39,'?VIDEOTYPE',39")
        }
    }
}
...
//
//---------------------------------------------------------------------------------
BUTTON_EVENT[dvTP, BTN_GET_ACTIVE_WINDOW]
{
    push:
    {
        if (bActiveComponent)
        {
            send_command vdvDev[nCurrentZone], '?ACTIVEWINDOW'
            println ("'send_command ',dpstoa(vdvDev[nCurrentZone]),', ',39,'?ACTIVEWINDOW',39")
        }
    }
}
...
//
//---------------------------------------------------------------------------------
BUTTON_EVENT[dvTP, BTN_VIDEO_TYPE_SECAM]
{
    push:
    {
        if (bActiveComponent)
        {
            send_command vdvDev[nCurrentZone], 'VIDEOTYPE-SECAM'
            println ("'send_command ',dpstoa(vdvDev[nCurrentZone]),', ',39,'VIDEOTYPE-SECAM',39")
        }
    }
}
...
//
//---------------------------------------------------------------------------------
BUTTON_EVENT[dvTP, BTN_VIDEO_TYPE_NTSC]
{
    push:
    {
        if (bActiveComponent)
        {
            send_command vdvDev[nCurrentZone], 'VIDEOTYPE-NTSC'
            println ("'send_command ',dpstoa(vdvDev[nCurrentZone]),', ',39,'VIDEOTYPE-NTSC',39")
        }
    }
}
...
//
//---------------------------------------------------------------------------------
BUTTON_EVENT[dvTP, BTN_VIDEO_TYPE_PAL]
{
    push:
    {
        if (bActiveComponent)
        {
            send_command vdvDev[nCurrentZone], 'VIDEOTYPE-PAL'
            println ("'send_command ',dpstoa(vdvDev[nCurrentZone]),', ',39,'VIDEOTYPE-PAL',39")
        }
    }
}
...
//
//---------------------------------------------------------------------------------
BUTTON_EVENT[dvTP, BTN_VIDEO_TYPE_AUTO]
{
    push:
    {
        if (bActiveComponent)
        {
            send_command vdvDev[nCurrentZone], 'VIDEOTYPE-AUTO'
            println ("'send_command ',dpstoa(vdvDev[nCurrentZone]),', ',39,'VIDEOTYPE-AUTO',39")
        }
    }
}
...
[dvTP,BTN_ACTIVE_WINDOW_RIGHT] = (sACTIVEWINDOW[nCurrentZone] == 'RIGHT')
[dvTP,BTN_VIDEO_TYPE_PAL] = (sVIDEOTYPE[nCurrentZone] == 'PAL')
[dvTP,BTN_VIDEO_TYPE_NTSC] = (sVIDEOTYPE[nCurrentZone] == 'NTSC')
[dvTP,BTN_VIDEO_TYPE_AUTO] = (sVIDEOTYPE[nCurrentZone] == 'AUTO')
[dvTP,PIP_FB] = [vdvDev[nCurrentZone],PIP_FB]
[dvTP,BTN_ACTIVE_WINDOW_MAIN] = (sACTIVEWINDOW[nCurrentZone] == 'MAIN')
[dvTP,BTN_ACTIVE_WINDOW_SUB] = (sACTIVEWINDOW[nCurrentZone] == 'SUB')
[dvTP,BTN_ACTIVE_WINDOW_LEFT] = (sACTIVEWINDOW[nCurrentZone] == 'LEFT')
[dvTP,BTN_VIDEO_TYPE_SECAM] = (sVIDEOTYPE[nCurrentZone] == 'SECAM')
