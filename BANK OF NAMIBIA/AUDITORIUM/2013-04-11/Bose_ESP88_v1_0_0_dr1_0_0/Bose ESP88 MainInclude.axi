PROGRAM_NAME='Bose ESP88 MainInclude'
(***********************************************************)
(*  FILE CREATED ON: 02/15/2007  AT: 12:48:05              *)
(***********************************************************)
(*  FILE_LAST_MODIFIED_ON: 06/09/2008  AT: 13:53:42        *)
(***********************************************************)
(***********************************************************)
(* System Type : NetLinx                                   *)
(***********************************************************)
...
([dvTPMain,nSubNavBtns[1]]..[dvTPMain,nSubNavBtns[length_array(nSubNavBtns)]], [dvTPMain,ZONE_PAGE])
...
data_event[dvTPMain]
{
    ONLINE:
    {
		...
		SEND_COMMAND dvTPMain,"'^ENA-',ITOA(ZONE_PAGE),',0'";
    }
}
...
button_event[dvTPMain, nSubNavBtns]
{
    PUSH:
    {
		...
		if (nActiveDevice == nDevice)
		{
			...
			SEND_COMMAND dvTPMain,"'^ENA-',ITOA(ZONE_PAGE),',1'";
...
button_event[dvTPMain,ZONE_PAGE]
{
	PUSH:
	{
		ON[dvTPMain, ZONE_PAGE]
		...
		if (bActiveComponent)
		{
			...
			SEND_COMMAND dvTPMain,"'^ENA-',itoa(nZoneBtns[1]),'..',itoa(nZoneBtns[LENGTH_ARRAY(nZoneBtns)]),',0'" 
			
			// Enable only those available for the current component
			SEND_COMMAND dvTPMain,"'^ENA-',itoa(nZoneBtns[1]),'..',itoa(nZoneBtns[LENGTH_ARRAY(vdvDev)]),',1'" 
		}
